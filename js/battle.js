...
const Battle = (() => {
  ...
  // ------------------------------------------------------ spawn-in anim
  const SPAWN_THEMES = {
    P: [[1, 1, 1], [1, 0.55, 0.8], [0.7, 0.9, 1]],
    E: [[1, 1, 1], [1, 0.85, 0.45], [1, 0.55, 0.2]],
  };
  ...
  function* spawnIn(a, theme) {
    const base = a.pos;
    Sfx.play('charge');
    // converging spiral of light around the spawn point
    for (let i = 0; i < 20; i++) {
    ...
    yield 90;
    // appear white-hot and grow from the feet up (origin is bottom-center)
    a.visible = true;
    ...
    Cam.kickFov(-2.5, 320);
    Fx.addTrauma(0.12);
    // glints shimmering across the voxel silhouette
    ...
    yield 240;
    // touchdown: ground ring + dust kick
  ...
  function* introScript() {
    ...
    yield* spawnIn(enemy, SPAWN_THEMES.E);
    yield 150;
    yield* say(BData.MSG.intro3, { auto: true, hold: 120 });
    yield* spawnIn(player, SPAWN_THEMES.P);
    yield 200;
    backToMenu();
  }
